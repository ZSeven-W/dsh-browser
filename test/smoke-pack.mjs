...
try {
  ...
  const probe = [
    ...
    "if (BROWSER_DRIVER_CONTRACT_VERSION !== 8) throw new Error('driver contract version mismatch');",
    "const manager = new BrowserManager({ allowedOrigins: [] });",
    "if (manager.kind !== 'browser' || manager.contractVersion !== 8) throw new Error('driver identity mismatch');",
    "try {",
