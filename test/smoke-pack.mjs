...
try {
  ...
  const probe = [
    ...
    "if (BROWSER_DRIVER_CONTRACT_VERSION !== 7) throw new Error('driver contract version mismatch');",
    "const manager = new BrowserManager({ allowedOrigins: [] });",
    "if (manager.kind !== 'browser' || manager.contractVersion !== 7) throw new Error('driver identity mismatch');",
    "try {",
